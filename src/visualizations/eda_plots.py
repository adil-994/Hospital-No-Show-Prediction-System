...
def plot_class_balance(df):
    """
    Visualizes class imbalance in the target variable.
    """
    ...
    print(f"No-show rate: {no_shows / total:.1%}")
...
def plot_waiting_impact(df):
    """
    Visualizes the impact of waiting time on attendance.
    """
    ...
    plt.savefig(
        'reports/figures/waiting_impact.png',
        bbox_inches='tight'
    )
...
def plot_weather_correlation(df):
    """
    Displays correlation between weather, waiting time,
    ...
    plt.title(
        'Correlation: Weather & Waiting Time vs No-Show',
        pad=20
    )
    ...
    plt.savefig(
        'reports/figures/weather_correlation.png',
        bbox_inches='tight'
    )
...
def run_full_eda():
    ...
    if not os.path.exists(data_path):
        print("Feature engineering output not found.")
        return
